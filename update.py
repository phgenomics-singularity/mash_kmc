...
@click.command()
@click.option("--mash_version", default=None)
...
def update_singularity(mash_version,kmc_version, author, config):
    ...
    if mash_version is not None:
        config['mash_version'] = mash_version
    if kmc_version is not None:
        config['kmc_version'] = kmc_version
    if author is not None:
        config['author'] = author
    
    loader = jinja2.FunctionLoader(load_template)
    env = jinja2.Environment(loader=loader)
    SINGULARITY_RECIPE = env.get_template("mash.singularity").render(config)
